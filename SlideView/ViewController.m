...
- (void)viewDidLoad {
    [super viewDidLoad];
    _textView1 = [[CATextLayerView alloc]initWithFrame:CGRectMake(0, 20.0f, [UIScreen mainScreen].bounds.size.width, 100.0f)];
    _textView1.string = @"Core Animation 提供了一个叫 CATextLayer 的 CALayer 子类，它包含了 UILabel 中所有图层组成的字符串绘制特性，还增添了一些额外的特性……。 CATextLayer 也比 UILabel 渲染快。它在iOS6及之前是不为人知的事实， UIlabel 实际上使用 WebKit 来进行文本绘制，这在你绘制大量文本时带来显著的性能负担";
    
    CATextLayerView *textView2 = [[CATextLayerView alloc]initWithFrame:CGRectMake(0, 20.0f + _textView1.frame.size.height, [UIScreen mainScreen].bounds.size.width, 100.0f)];
